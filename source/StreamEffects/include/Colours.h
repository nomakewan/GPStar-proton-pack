...
inline uint32_t maximizeBrightness(uint32_t rgb, uint8_t limit = 255)  {
  ...
  return device_led_output.Color(red, green, blue);
}
...
uint32_t nscale8(uint32_t colour, uint8_t scale) {
  ...
  return device_led_output.Color(r, g, b);
}
...
uint32_t getHue(uint8_t i_device, uint8_t i_colour, uint8_t i_brightness = 255, uint8_t i_saturation = 255) {
  ...
  switch(i_colour) {
    ...
    case C_ORANGE_FADE:
      ...
      i_output_colour = 28;
      i_saturation = 255;
      i_brightness = i_curr_bright[i_device];
    break;
  ...
  return device_led_output.ColorHSV(getHue16(i_output_colour), i_saturation, i_brightness);
}

uint32_t getHueAsRGB(uint8_t i_device, uint8_t i_colour, uint8_t i_brightness = 255, bool b_grb = false) {
  ...
  if(b_grb) {
    ...
    uint32_t grb = device_led_output.Color(g, r, b);
    return device_led_output.gamma32(grb);
  }
  else {
    return device_led_output.gamma32(rgb); // Return RGB object.
  }
}
...
uint32_t getHueAsGBR(uint8_t i_device, uint8_t i_colour, uint8_t i_brightness = 255) {
  ...
  uint8_t g = (uint8_t)(rgb >>  8);
  uint8_t b = (uint8_t)(rgb >>  0);
  uint32_t gbr = device_led_output.Color(g, b, r);

  // Swap colour values before returning.
  return device_led_output.gamma32(gbr);
}
